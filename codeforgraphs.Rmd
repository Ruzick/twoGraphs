...
z<-read.table(file = 'country_country_aug2020.tsv', sep = '\t', header = TRUE)
percountrydb<-tbl_df(z)
View( percountrydb)

s<-spread(percountrydb,fr_loc,scaled_sci)
s<-s[-nrow(s),] #Remove NA

#Higuest connections
conn_high<-arrange(percountrydb,desc(scaled_sci))
conn_high<-conn_high[-40,]
View(conn_high)
#lets choose the countries with the highest connections inside the country

conWit<-conn_high[1:89,]
#Lets pick the 10 highest to pop up some visuals
conWit10<- conWit[1:11,]
conWit10<-conWit10[,-2] #remove the extra column
ggplot(conWit10, aes(x=user_loc, y=scaled_sci)) + geom_point()


#Lets try 20

#Lets pick the 20 highest to pop up some visuals
conWit20<- conWit[1:21,]
conWit20<-conWit20[,-2] #remove the extra column
ggplot(conWit20, aes(x=user_loc, y=scaled_sci)) + geom_point()


#From the graph abpve, we notice these are all small countries, most of them islands
#Below one can compare
u<-read.delim(file = '10fulldat.txt', sep = '\t', header = TRUE)
t<-tbl_df(u)
summary(t)
pairs(t)
#From here below, code was borrowed from the database creator for connectedness
dir.sci_dat_country <- 'country_country_aug2020.tsv'

# Read in the country SCI data
sci_dat <- read_tsv(dir.sci_dat_country)
#sci_dat <- rename(sci_dat, sci=scaled_sci)
names(sci_dat)[names(sci_dat) == "scaled_sci"] <- "sci"
...
for(i in 1:length(regions)){
  ...
  # Save output to the folder "output/country_output"   had previously "output/sci_"
  ggsave(paste0("output", regions[i], ".jpg"),
